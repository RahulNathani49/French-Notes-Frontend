...
const ManageContentSection = ({
    ...
    // Update editForm.text when contentEditable changes
    const handleDivInput = () => {
        setEditForm(prev => ({ ...prev, text: editableDivRef.current.innerHTML }));
    };

    useEffect(() => {
        if (editingId && editableDivRef.current) {
        ...
        <div className="py-5">
            ...
            {editingId && (
                <div className="card p-3 mb-3 w-100">
                    <h4>Edit Content</h4>
                    <input type="text" name="title" className="form-control mb-2" value={editForm.title}
                           onChange={(e) => handleEditChange(e, setEditForm)} placeholder="Title"/>
                    <select name="type" className="form-select mb-2" value={editForm.type}
                    ...
                    <label className="form-label">Content Text (Rich HTML)</label>
                    <div
                        ref={editableDivRef}
                        contentEditable
                    ...
                    <input type="file" accept="image/*" className="form-control mb-2"
                           onChange={(e) => setEditForm(prev => ({...prev, imageFile: e.target.files[0]}))}/>
                    <label className="form-label">Upload Audio</label>
                    <input type="file" accept="audio/*" className="form-control mb-3"
                           onChange={(e) => setEditForm(prev => ({...prev, audioFile: e.target.files[0]}))}/>
                    <div className="d-flex gap-2">
                        <button type="button" className="btn btn-primary"
                                onClick={() => handleUpdateContent(editingId, editForm, setContents, setEditingId, setEditForm, setContentError)}>Save
                        </button>
                        <button type="button" className="btn btn-secondary" onClick={() => setEditingId(null)}>Cancel
                        </button>
                    </div>
                </div>
            )}
            ...
            <div className="table-responsive">
                <table className="table table-bordered">
                    ...
                    {filteredContents.length > 0 ? (
                        ...
                        <tr>
                            <td colSpan="6" className="text-center text-muted py-4">No content found matching the filters.</td>
                        </tr>
                    )}
                    </tbody>
                </table>
